...
const ProductActions = ({ item, itemCount, setItemCount = null, relative = false }) => {
    ...
    return (
        <div className={`ease_transition max-w-12 flex flex-col items-center ${!relative ? 'absolute top-0 -right-4' : 'relative'}  bg-primary-100 rounded overflow-hidden `}>

            <div onClick={() => cartActions(itemCount <= 0 ? 'add' : 'update-add')} className='w-full h-8 flex justify-center items-center bg-primary-500'>
            ...
            {itemCount > 0 && <div onClick={() => cartActions(itemCount <= 1 ? 'remove' : 'update-remove')} className='w-full h-8 flex justify-center items-center bg-primary-500'>
                {
